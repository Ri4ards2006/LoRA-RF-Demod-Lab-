...
module iq_demod (
    ...
    always @(posedge clk_27m) begin
        if (!rst_n) begin
            ...
            if (dec_cnt == DEC_FACTOR - 1) begin
                dec_cnt <= 0;
                i_led <= $unsigned(i_sum[9:2]);
                q_led <= $unsigned(q_sum[9:2]);
            end else begin
                dec_cnt <= dec_cnt + 1;
                i_led <= i_led;
                q_led <= q_led;
    ...
    always @(posedge clk_27m) begin
        if (!rst_n) begin
            ...
            reg signed [7:0] i_abs;
            reg signed [7:0] q_abs;
            reg [15:0] mag_sq;
            i_abs = i_led[7] ? -i_led : i_led;
            q_abs = q_led[7] ? -q_led : q_led;
            mag_sq = (i_abs * i_abs) + (q_abs * q_abs);
            mag_led <= mag_sq[15:12];
        end
    end
